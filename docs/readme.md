## Functionality
* [List Directory](list_directory.md)
* [Registry query](registry.md)
* [Disk Info](diskinfo.md)
* [Http Headers](http_headers.md)

## Manifest Information
* [Manifest content by manifest name](manifest_content.md)
* [Manifest content by collected file](manifest_by_file.md)

## Tooling
* [Parse manifests](tools.md#Parse-manifests)
 
## Tests
* [Inspection Tests](tests.md#Inspection-Tests)
* [Manifest documentation test](tests.md#manifest-documentation-test)

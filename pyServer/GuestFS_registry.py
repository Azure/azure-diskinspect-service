...
class GuestFS_Registry:
    ...
    def open_hive(self, hiveName):
        try:
            ...
            if len(regHivePath) == 0:
                return False;
            (out, err) = self.guestFS.callGF('Opening registry hive [' + regHivePath + ']', ['--', '-hivex-open', regHivePath], True, retryCount=1)
    ...
    def close_hive(self):
        try:
            if self.current_open_hive != None:
                (out, err) = self.guestFS.callGF('Closing registry hive', ['--', '-hivex-close'], True, retryCount=1)
                self.current_open_hive = None
                if err:
                    return False
        except subprocess.CalledProcessError:
        ...
        try:
            ...
            for index in range(1, len(reg_nodes)-1):  
                ...
                if (not current_key_path in self.regCache):
                    if (index == 1):
                        # Hive root
                        (nodeid, err) = self.guestFS.callGF('Get registry root', ['--', '-hivex-root'], True, retryCount=1)
                    else:                        
                        if ( current_key_path.casefold() == "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet".casefold() ):
                            ...
                            (nodeid, err) = self.guestFS.callGF('Get registry child: [' + reg_nodes[index] + ']', ['--', '-hivex_node_get_child', current_node_id, reg_nodes[index]], True, retryCount=1)
    ...
    def get_string_value_by_name(self, node_id, value_name):
        ...
        (valueid, err) = self.guestFS.callGF('Get registry value-id: [' + value_name + ']', ['--', '-hivex-node-get-value', node_id, value_name], True, retryCount=1)
        if err or not valueid or valueid[0] == 0:
            return None
        # get the type for the value
        (valueType, err) = self.guestFS.callGF('Get registry value-type: [' + value_name + ']', ['--', '-hivex-value-type', valueid[0]], True, retryCount=1)
    ...
    def determine_currentcontrolset(self, system_node_id):        
        ...
        (nodeid, err) = self.guestFS.callGF('Get registry child: [determine_currentcontrolset()]: HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Select', ['--', '-hivex_node_get_child', system_node_id, "Select"], True, retryCount=1)
        if err or (nodeid is None) or len(nodeid)== 0 or (nodeid[0] == 0):
        ...
        (nodeid, err) = self.guestFS.callGF('Get registry child: [' + actual_controlset + ']', ['--', '-hivex_node_get_child', system_node_id, actual_controlset], True, retryCount=1) 
        return nodeid
